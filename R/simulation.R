...
#'
#' @export
sim_2x2 <- function(n_subj = 48, n_obs = 48, n_items = 48,
                    int = 0, A = 0, B = 0, AB = 0,
  ...
  if (n_obs %% n_items)
    warning("Are you sure 'n_items' shouldn't be a factor of 'n_obs'?")
  ...
  ## sfx <- make_rfx(n_subj, subj_rint, subj_rslp, subj_rcorr)
  ## ifx <- make_rfx(n_items, item_rint, item_rslp, item_rcorr)
  ...
  cols <- c("subj_id", "item_id", "A", "B", "A_c", "B_c", "tnum_r", "tnum_b", "Y_r", "Y_b")
...
## sim_2x2 <- function(n_subj = 48, n_obs = 48, n_items = 48,
##                     int = 0, A = 0, B = 0, AB = 0,
##                     item_rint = .5, item_rslp = .5,
##                     item_rcorr = .5, subj_rint = .5,
##                     subj_rslp = .5, subj_rcorr = .5,
##                     version = 0L,verbose = FALSE){
...
##   make_rfx <- function(n, rint, rslp, rcorr){
##     rfx_mx <- matrix(c(rint,
##                        sqrt(rint) * sqrt(rslp) * rcorr,
##                        sqrt(rint) * sqrt(rslp) * rcorr,
##                        rslp), nrow = 2)
